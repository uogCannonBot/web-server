...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        res.status(400).json({
            error: `failed to get webhooks with user id: ${id}`,
        });
        throw err;
    } finally {
...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        res.status(404).json({
            success: false,
            message:
                "the webhook you are trying to create does not exist in Discord",
            error: err,
        });
    }
...
router.delete("/:webhookId", async (req, res) => {
    ...
    try {
        ...
        if (
            optionsHeader.affectedRows === 0 ||
            webhookHeader.affectedRows === 0
        ) {
            res.status(404).json({
                error: `record with webhook id: ${webhookId} does not exist`,
            });
        }
    } catch (err) {
        res.status(409).json({
